...
void TestBoard::move_pawnSimple()
{
   ...
   Board board(nullptr, true /*noreset*/);
   board.numMoves = 17;
   board.board[0][2] = new PieceSpy(0, 2, true  /*isWhite*/, PAWN);
   board.board[0][3] = new PieceSpy(0, 3, false /*isWhite*/, SPACE);
   board.board[0][2]->nMoves = 17;
   PieceSpy::reset();

   // EXERCISE
   board.move(move);
   // VERIFY
   assertUnit(board.numMoves == 18);
...
void TestBoard::construct_default()
{
   ...
   // EXERCISE
   // set up the board by calling reset the board
   board.reset();
   // VERIFY
   ...
   for (int x = 0; x < 8; x++) {
      assertUnit(PAWN == (board.board[x][1])->getType());
      assertUnit(PAWN == (board.board[x][6])->getType());
   }
   // TEARDOWN
   for (int col = 0; col < 8; col++)
   {
      ...
      {
         delete board.board[col][row];
         board.board[col][row] = nullptr;
      }
   }
}
